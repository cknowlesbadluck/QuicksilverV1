...
/// The store intentionally persists metadata and execution state, never credentials.
public actor IntegrationTaskStore {
    ...
    public struct TaskStep: Codable, Sendable, Hashable, Identifiable {
        ...
        public init(order: Int, capability: IntegrationCapability, connectorID: String, provider: IntegrationProvider, status: TaskStatus = .queued) {
            self.id = UUID()
            self.order = order
    ...
    public init(fileURL: URL? = nil) {
        let base = fileURL ?? Self.defaultURL()
        self.fileURL = base
        self.tasks = Self.load(from: base)
    }
    ...
    public func task(id: UUID) -> Task? { tasks[id] }
    ...
    private func persist() throws {
        ...
        let data = try JSONEncoder.quicksilver.encode(tasks)
        try data.write(to: fileURL, options: .atomic)
    }

    private static func load(from url: URL) -> [UUID: Task] {
        guard let data = try? Data(contentsOf: url),
              let tasks = try? JSONDecoder.quicksilver.decode([UUID: Task].self, from: data) else { return [:] }
        return tasks
    }
...
private extension JSONEncoder {
    static var quicksilver: JSONEncoder {
...
private extension JSONDecoder {
    static var quicksilver: JSONDecoder {
