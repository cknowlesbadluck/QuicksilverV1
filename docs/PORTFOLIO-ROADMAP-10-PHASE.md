...
2. **Owner: set resonancenexus env + schema** — Exit: `GET https://resonancenexus.netlify.app/api/ready` 200.
3. **Prove durable GitHub vertical slice** — member-only evidence after restart; deny proofs.
4. **Device HG CHR-55** — Archive IPA on iPhone 16e from QuicksilverV1 `7e15be24` or later. GitHub Actions success is not acceptance.
...
7. **Conduit #125 keyset pagination** — merge only after rebase onto current main and required checks are green.
