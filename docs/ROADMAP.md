...
| M3-T20 | Router: selection, failover, timeouts, cancellation, `/v1/config` | gateway/src/router.ts, gateway/config/routing.json | Each tier has an ordered candidate list (provider, model, `trainsOnPrompts`, `dailyBudget`, `maxOutputTokens`). Failover happens on 429 / 5xx / upstream first-byte timeout **only before the first delta**. A client abort cancels the upstream request through `AbortController`. A candidate whose secret is missing is skipped, which makes xAI optional. `GET /v1/config` returns client-safe routing only. Vitest with the fake covers failover order, no failover after the first delta, abort propagation and a skipped optional provider. | M | M3-T16, M3-T18, M3-T19 |
...
| M4-T7 | Accurate Privacy Manifest | PrivacyInfo.xcprivacy, ci.yml Structure (validate plist with `python3 -c plistlib`) | Declares crash data, performance data and other user content (the trimmed question/context sent through the gateway to AI providers) and audio data is **not** collected (speech is on-device), not linked and not used for tracking. The plist parses in CI. | S | — |
...
**HUMAN GATE HG6 (after M4-T9):** confirm the repo secret `SENTRY_AUTH_TOKEN` is set (org `inbetween`, project `quicksilver`). Run the Archive, trigger a test crash (see M6-T3) and confirm a **symbolicated** event arrives. Then turn the opt-out on and confirm no events arrive.
...
| ID | Task | Likely files | Acceptance | Size | Deps |
|---|---|---|---|---|---|
| M6-T1 | Harden the Archive checks | .github/workflows/archive.yml | Missing persona prompts or PrivacyInfo becomes an **error**. Verify `Sentry.framework` is embedded, and that Info.plist version/build match `project.yml`. (Workflow change only; the owner runs it in HG8.) | S | M4-T10 |
...
| M6-T3 | Diagnostic test-event action | UI/CodexView.swift ("Record" section), App/CrashReporting.swift | A "Send diagnostic event" button (and a test crash behind a confirm, Release included) exists for HG6. AppTests cover the action with a stub. | S | M4-T8 |
...
| M3.5 Apple Intelligence & App Intents | 22 (3 moved from M5; 4 added for on-device planning, the action registry, summaries and voice) |
| M4 Memory, data & privacy | 11 |
| M5 Experience completion | 10 (3 moved out, 2 Ask tasks added) |
| M6 Release readiness | 6 |
| **Total** | **97** (96 open) + 9 human gates (HG1–HG9) and the #135 merge |
