...
- **Single entity.** No file says "You are Forge" or "You are Eternal". Aspects are "moods" or "aspects of you". This matches AGENTS.md: "Forge and Eternal are aspects, not separate personas" (an aspect may be pinned per thread as a mood; Auto is the default).
...
- **Owner name.** The prompts say `{{owner}}`. `PromptComposer` (P-T1) substitutes "Christopher" for on-device composition and "the owner" for cloud-bound prompts, so the name never reaches a provider (M3-T11). Token counts are measured with the longest value substituted.
