...
final class IntegrationPlaneTests: XCTestCase {
    func testProviderAndCapabilityCodableRoundTrip() throws {
        let connector = IntegrationConnector(id: "dev.github", provider: .github, capabilities: [.repo, .issues, .pullRequests], transport: .oauth, credential: .oauth, enabled: true)
        let decoded = try JSONDecoder().decode(IntegrationConnector.self, from: JSONEncoder().encode(connector))
        XCTAssertEqual(decoded, connector)
    }

    func testIntegrationPlanStepUsesWireKeys() throws {
        let step = IntegrationPlanStep(order: 1, capability: .pullRequests, connectorID: "dev.github", provider: .github, approvalRequired: true)
        let object = try XCTUnwrap(JSONSerialization.jsonObject(with: JSONEncoder().encode(step)) as? [String: Any])
    ...
    func testAnyCodableRoundTrip() throws {
        let value: AnyCodable = .object(["name": .string("Quicksilver"), "enabled": .bool(true), "count": .int(3), "items": .array([.string("GitHub"), .string("Linear")])])
        let decoded = try JSONDecoder().decode(AnyCodable.self, from: JSONEncoder().encode(value))
    ...
    func testTaskStorePersistsAndReloads() async throws {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("quicksilver-task-\(UUID().uuidString).json")
    ...
    func testEventStoreIsRecoverable() async throws {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("quicksilver-events-\(UUID().uuidString).json")
        defer { try? FileManager.default.removeItem(at: url) }
        let taskID = UUID()
