    ...
    private func realmHeader(_ vm: EternalViewModel, accent: Color, radius: CGFloat) -> some View {
        HStack(spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("THE ETERNAL").font(.caption.weight(.bold)).tracking(1.8).foregroundStyle(PersonaTheme.mercurySilver)
        ...
        .overlay(RoundedRectangle(cornerRadius: radius, style: .continuous).strokeBorder(accent.opacity(0.4), lineWidth: 1))
    }

    private func awakenCard(_ vm: EternalViewModel, accent: Color, radius: CGFloat) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Mercury is listening.").font(.headline).foregroundStyle(PersonaTheme.mercurySilver)
            Text("Awaken the observational field: patterns, continuity, diagnostics, memory.").font(.caption).foregroundStyle(.secondary)
            Button { Task { await vm.awakenEternal() } } label: {
                Text("Awaken Eternal").font(.subheadline.weight(.semibold)).frame(maxWidth: .infinity).padding(.vertical, 12)
