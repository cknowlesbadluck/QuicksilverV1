        ...
        TimelineView(.animation(minimumInterval: reduceMotion ? 1 / 8 : 1 / 30)) { timeline in
            Canvas { context, size in
                ...
                for index in 0..<lineCount {
                    let y = size.height * (0.15 + Double(index) * (0.7 / Double(max(lineCount, 1))))
                    var path = Path()
                    path.move(to: CGPoint(x: 0, y: y + sin(time * 0.4 + Double(index)) * 8 * intensity))
                    path.addLine(to: CGPoint(x: size.width, y: y + cos(time * 0.35 + Double(index)) * 8 * intensity))
                    context.stroke(
                        path,
