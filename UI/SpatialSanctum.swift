...
enum SpatialDestination: String, CaseIterable, Identifiable {
    ...
    /// Accent derived from aspect / visual language. Not a product switcher color.
    var accent: Color {
        switch self {
    ...
    /// Aspect bias for this place. Brain still owns the active aspect;
    /// this is only the preferred expression bias when the place is entered.
    var preferredAspect: Aspect {
        switch self {
        case .workshop: return .forge
        case .planetarium: return .eternal
        case .archive, .codex, .diagnostics: return .quicksilver
        }
    }
}
...
///
/// Deliberately 2.5D SwiftUI composition — not a game-engine scene.
/// Depth, parallax, portals, and atmospheric motion can evolve independently
/// of the underlying functional views.
