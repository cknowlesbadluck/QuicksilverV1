...
struct ForgeView: View {
    ...
    private func realmHeader(_ vm: ForgeViewModel, accent: Color, radius: CGFloat) -> some View {
        ...
        .padding(16)
        .background(
            .ultraThinMaterial.opacity(0.52),
    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: reduceMotion ? 1 / 8 : 1 / 30)) { timeline in
            ...
            ZStack {
                Circle().fill(
                    .radialGradient(
                        colors: [PersonaTheme.mercurySilver.opacity(0.9), accent.opacity(0.35), .clear],
                        center: .center,
                        startRadius: 1,
                        endRadius: size * 0.7
                    )
                ...
                Capsule().fill(
                    .linearGradient(
                        colors: [PersonaTheme.mercurySilver, accent.opacity(0.5), PersonaTheme.voidBlack],
        ...
        TimelineView(.animation(minimumInterval: reduceMotion ? 1 / 8 : 1 / 30)) { timeline in
            Canvas { context, size in
                ...
                for stepIndex in 0..<7 {
                    let phase = Double(stepIndex) * 0.87
                    let radiusValue = maxRadius * (0.20 + Double(stepIndex) * 0.085)
                    let xPos = center.x + cos(timeValue * (0.04 + Double(stepIndex) * 0.006) + phase) * radiusValue * 0.32
                    let yPos = center.y + sin(timeValue * (0.05 + Double(stepIndex) * 0.004) + phase) * radiusValue * 0.18
                    let rect = CGRect(x: xPos - 1.2, y: yPos - 1.2, width: 2.4, height: 2.4)
