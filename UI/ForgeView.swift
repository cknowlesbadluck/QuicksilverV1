...
/// The Workshop — Forge chamber for creation, engineering, architecture, experiments.
/// Instruments are live Nexus readings; intelligence still flows only through MercuryBrain.
struct ForgeView: View {
    ...
    private func forgeContent(_ vm: ForgeViewModel) -> some View {
        let accentID = vm.activeAspect == .forge ? "forge" : vm.activePersonaID
        let accent = PersonaTheme.accent(for: accentID)
        let radius = PersonaTheme.cardCornerRadius(for: accentID)
        let spacing = 16 * PersonaTheme.density(for: accentID)
        ...
        .animation(PersonaTheme.spring(for: accentID), value: vm.sessionNotes.count)
        .animation(PersonaTheme.spring(for: accentID), value: vm.instruments.map(\.severity))
    }
    ...
    private func realmHeader(_ vm: ForgeViewModel, accent: Color, radius: CGFloat) -> some View {
        HStack(spacing: 14) {
            ...
            Spacer()
            Text(vm.isAwake ? "FORGING" : "DORMANT")
    ...
    private func instrumentsPanel(_ vm: ForgeViewModel, accent: Color, radius: CGFloat) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                ...
                Button {
                    ...
                    lastAnswer = nil
                    Task {
                        let result = await vm.runDiagnosticInstrument()
                        lastAnswer = result
                        isDiagnosing = false
                    }
                } label: {
                    HStack(spacing: 4) {
                        if isDiagnosing { ProgressView().scaleEffect(0.7) }
                        Text(isDiagnosing ? "Reading…" : "Diagnose")
            ...
            HStack(spacing: 8) {
                ForEach(vm.instruments) { instrument in
                    instrumentTile(instrument, radius: radius)
                }
            }
        }
    }

    private func instrumentTile(_ instrument: ForgeViewModel.InstrumentReading, radius: CGFloat) -> some View {
        let color: Color = {
            switch instrument.severity {
            case .critical: return PersonaTheme.hazardGreen
            case .elevated: return PersonaTheme.toxicGreen
            case .nominal: return PersonaTheme.mercurySilver
            }
        }()

        return VStack(spacing: 3) {
            Text(instrument.label.uppercased())
        ...
        .padding(.vertical, 9)
        .background(
            .ultraThinMaterial.opacity(instrument.severity == .nominal ? 0.3 : 0.42),
        ...
        .overlay(
            RoundedRectangle(cornerRadius: max(8, radius - 4), style: .continuous)
                .strokeBorder(color.opacity(instrument.severity == .nominal ? 0 : 0.35), lineWidth: 1)
        )
    }
    ...
    private func constructiveAsk(_ vm: ForgeViewModel, accent: Color, radius: CGFloat) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("ASK THE WORKSHOP").font(.caption2.weight(.bold)).foregroundStyle(PersonaTheme.glowPurple)
...
private struct MercuryDroplet: View {
    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: reduceMotion ? 1 / 8 : 1 / 30)) { timeline in
            ...
            let wobble = reduceMotion ? 0 : sin(time * (active ? 1.7 : 0.5)) * 2.5
            ZStack {
                ...
                Capsule().fill(.linearGradient(colors: [PersonaTheme.mercurySilver, accent.opacity(0.5), PersonaTheme.voidBlack], startPoint: .topLeading, endPoint: .bottomTrailing))
                    ...
                    .shadow(color: accent.opacity(0.55), radius: active ? 10 : 4)
            }
        }
        ...
        TimelineView(.animation(minimumInterval: reduceMotion ? 1 / 8 : 1 / 30)) { timeline in
            Canvas { context, size in
                ...
                for i in 0..<7 {
                    ...
                    let x = center.x + cos(time * (0.04 + Double(i) * 0.006) + phase) * r * 0.32
                    let y = center.y + sin(time * (0.05 + Double(i) * 0.004) + phase) * r * 0.18
